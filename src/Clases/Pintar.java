...
public class Pintar {   
   ...
   public static void pintarCirculo(Graphics g,int x,int y,String n){
       ...
       ((Graphics2D)g).setColor(Color.white);
        ((Graphics2D)g).setStroke(new BasicStroke(2));//Grosor del circulo       
  ...
  public static void pintarCamino(Graphics g, int x1,int y1,int x2,int y2, Color color, String n, String p, String tam){
      ((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING,  RenderingHints.VALUE_ANTIALIAS_ON);
        BasicStroke stroke = new BasicStroke(2);
        ((Graphics2D)g).setStroke(stroke);
        g.setColor(color);
        ...
        Font fuente=new Font("Monospaced",Font.BOLD, 16);
        g.setFont(fuente);
        ((Graphics2D)g).drawString(n, x1, y1);
        ((Graphics2D)g).drawString(p, x2, y2);
